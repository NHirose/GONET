...
model_file_gen = 'nn_model/featlayer_gen_stereo.h5'
model_file_dis = 'nn_model/featlayer_dis_stereo.h5'
model_file_invg = 'nn_model/featlayer_invg_stereo.h5'
model_file_fl = 'nn_model/classlayer_ts.h5'
...
class FL(chainer.Chain):
    ...
    def __call__(self, img_error, dis_error, dis_output, test=False):
        h = F.reshape(F.absolute(img_error), (img_error.data.shape[0], 6*128*128))
        h = self.l_img(h)
...
if __name__ == '__main__':
    ...
    msg1_sub = message_filters.Subscriber('/cv_camera_node1/image_raw', Image) #left side camera
    msg2_sub = message_filters.Subscriber('/cv_camera_node2/image_raw', Image) #right side camera
